...
namespace Microsoft.CodeAnalysis.IL
{
    internal class CSharpDeclarationWriter
    {
        ...
        private void WriteNamespaceDeclaration(INamespaceSymbol @namespace, SyntaxWriter writer)
        {
        ...
        private void WriteTypeDeclaration(INamedTypeSymbol type, SyntaxWriter writer)
        {
        ...
        private void WriteClassDeclaration(INamedTypeSymbol type, SyntaxWriter writer)
        {
            ...
            var implementsInterfaces = type.Interfaces.Where(t => t.IsVisibleOutsideAssembly()).Any();
            ...
            if (hasBaseType || implementsInterfaces)
            {
                ...
                if (implementsInterfaces)
                {
                    ...
                    foreach (var @interface in type.GetApiInterfaces())
                    {
                        if (isFirst)
                            isFirst = false;
                        else
        ...
        private void WriteStructDeclaration(INamedTypeSymbol type, SyntaxWriter writer)
        {
            ...
            if (type.Interfaces.Where(t => t.IsVisibleOutsideAssembly()).Any())
            {
                writer.WriteSpace();
                writer.WritePunctuation(":");
        ...
        private void WriteFieldDeclaration(IFieldSymbol field, SyntaxWriter writer)
        {
        ...
        private void WriteEnumFieldDeclaration(IFieldSymbol field, SyntaxWriter writer)
        {
            WriteAttributeList(field.GetAttributes(), writer);
            writer.WriteReference(field, field.Name);
            writer.WriteSpace();
        ...
        private void WriteMethodDeclaration(IMethodSymbol method, SyntaxWriter writer)
        {
        ...
        private void WriteAttributeList(ImmutableArray<AttributeData> attributes, SyntaxWriter writer)
        {
            WriteAttributeList(attributes, null, writer);
        }

        private void WriteAttributeList(ImmutableArray<AttributeData> attributes, string? target, SyntaxWriter writer)
        {
        ...
        private void WriteTypedConstant(TypedConstant constant, SyntaxWriter writer)
        {
            Debug.Assert(constant.Type != null);
        ...
        private void WriteConstant(ITypeSymbol type, object? value, SyntaxWriter writer)
        {
            if (value == null)
            {
                if (type.IsValueType)
        ...
        private void WriteAccessibility(Accessibility accessibility, SyntaxWriter writer)
        {
        ...
        private void WriteTypeParameterList(ImmutableArray<ITypeParameterSymbol> typeParameters, SyntaxWriter writer)
        {
        ...
        private void WriteConstraints(ImmutableArray<ITypeParameterSymbol> typeParameters, SyntaxWriter writer)
        {
            if (!typeParameters.Any())
                return;

            void WriteConstructorConstraint(SyntaxWriter writer)
            {
            ...
            void WriteNotNullConstraint(SyntaxWriter writer)
            {
                writer.WriteKeyword("notnull");
            }

            void WriteReferenceTypeConstraint(SyntaxWriter writer)
            {
                writer.WriteKeyword("class");
            }

            void WriteUnmanagedTypeConstraint(SyntaxWriter writer)
            {
                writer.WriteKeyword("unmanaged");
            }

            void WriteValueTypeConstraint(SyntaxWriter writer)
            {
                writer.WriteKeyword("struct");
            }
        ...
        private void WriteTypeReference(ITypeSymbol type, SyntaxWriter writer)
        {
            switch (type.TypeKind)
            {
                case TypeKind.Class:
        ...
        private void WriteTypeParameterReferences(ImmutableArray<ITypeSymbol> typeParameters, SyntaxWriter writer)
        {
            if (!typeParameters.Any())
                return;
        ...
        private void WriteParameterList(ImmutableArray<IParameterSymbol> parameters, SyntaxWriter writer)
        {
            writer.WritePunctuation("(");
        ...
        private void WriteParameters(ImmutableArray<IParameterSymbol> parameters, SyntaxWriter writer)
        {
